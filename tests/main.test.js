...
describe('Timer class test suite', () => {
	...
	describe('error handling', () => {
		...
		test('throws an error if you attempt to start twice', () => {
			expect(() => timer.start().start()).toThrowError(Error);
		});
		test('throws an error if you attempt to stop twice', () => {
			expect(() => timer.stop().stop()).toThrowError(Error);
		});
		test('throws an error if you try to operate after closure', () => {
			expect(() => timer.close().start()).toThrowError(Error);
			expect(() => timer.close().stop()).toThrowError(Error);
		});
		test('throws an error if specified a non-string as a label', () => {
			expect(() => new Timer(5)).toThrowError(TypeError);
			expect(() => new Timer(true)).toThrowError(TypeError);
			expect(() => new Timer(false)).toThrowError(TypeError);
			expect(() => new Timer({})).toThrowError(TypeError);
			expect(() => new Timer([])).toThrowError(TypeError);
		});
